...
- Prefer typed interfaces and context-based state.
- Match the backend contract exactly; endpoints use query parameters for IDs and filters.
- Keep Sentry instrumentation intact and do not remove `withSentryConfig`.
- Add features under the existing route groups and ensure public, admin, and student experiences remain separated.
- Use `useEffect` in page components only for lifecycle-triggered provider loads.
- Avoid adding global state outside providers unless it is truly UI-only.
- Reuse shared constants from `src/lib/common/constants.tsx`.

## Quick References
- Root provider composition: `app/(public)/layout.tsx`
- Shared Axios logic: `src/lib/utils/axiosInstance.ts`
- Common constants: `src/lib/common/constants.tsx`
- Provider pattern examples:
  - `src/providers/student-provider`
  - `src/providers/course-provider`
  - `src/providers/application-provider`
  - `src/providers/contact-provider`
  - `src/providers/address-provider`

---

This guide is intended as the primary reference for future code generation and feature work in the `next-ts` repository.
